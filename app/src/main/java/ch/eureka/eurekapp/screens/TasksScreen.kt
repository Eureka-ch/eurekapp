...
/** Horizontal filter bar with standard and project filters. */
@Composable
private fun FilterBar(
    uiState: TaskScreenUiState,
    selectedFilter: TaskScreenFilter,
    setFilter: (TaskScreenFilter) -> Unit
) {
  LazyRow(
  ...
  Column(
      ...
      verticalArrangement = Arrangement.Center) {
        Text(
            ...
            modifier = Modifier.padding(Spacing.md).testTag(TasksScreenTestTags.ERROR_MESSAGE))
      }
}

/** Task list with sections and empty state. */
@Composable
private fun TaskList(
  ...
  val currentTaskAndUsers = tasksAndUsers.filter { it.task.status != TaskStatus.COMPLETED }
  val completedTaskAndUsers = tasksAndUsers.filter { it.task.status == TaskStatus.COMPLETED }
        ...
        if (!isConnected) {
          item {
            Text(
                ...
                color = MaterialTheme.colorScheme.error,
                modifier =
                    Modifier.padding(Spacing.md).testTag(TasksScreenTestTags.OFFLINE_MESSAGE))
          }
        }
        taskSection(
            title = "Current Tasks",
