...
fun FilesManagementScreen(
    ...
    viewModel: FilesManagementViewModel = run {
        ...
        remember {
            FilesManagementViewModel(AppDatabase.getDatabase(context).downloadedFileDao(), context.applicationContext as android.app.Application)
        }
    }
) {
    ...
    Scaffold(
        topBar = { EurekaTopBar(title = "Downloaded Files", navigationIcon = { BackButton(onClick = onBackClick) }, modifier = Modifier.testTag(FilesManagementScreenTestTags.TOP_BAR_TITLE)) },
        content = { padding ->
            Column(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
                if (files.isEmpty()) {
                    Text("No downloaded files.", style = MaterialTheme.typography.bodyLarge, modifier = Modifier.testTag(FilesManagementScreenTestTags.NO_FILES_MESSAGE))
                } else {
                        ...
                        items(files) { fileItem ->
                            Row(modifier = Modifier.fillMaxSize(), horizontalArrangement = Arrangement.SpaceBetween) {
                                Row(modifier = Modifier.weight(1f), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                    ...
                                    Column {
                                        Text(fileItem.displayName, style = MaterialTheme.typography.bodyLarge, modifier = Modifier.testTag("${FilesManagementScreenTestTags.FILE_DISPLAY_NAME}_${fileItem.displayName}"))
                                    }
                                }
                                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                    Button(onClick = { viewModel.openFile(fileItem) }, modifier = Modifier.testTag(FilesManagementScreenTestTags.OPEN_BUTTON)) {
                                        Text("Open")
                                    }
                                    Button(onClick = {
    ...
    if (showDeleteDialog && fileToDelete != null) {
        AlertDialog(
            ...
            title = { Text("Confirm Deletion", modifier = Modifier.testTag(FilesManagementScreenTestTags.DELETE_DIALOG_TITLE)) },
            text = { Text("Are you sure you want to delete '${fileToDelete!!.displayName}'?", modifier = Modifier.testTag(FilesManagementScreenTestTags.DELETE_DIALOG_TEXT)) },
            confirmButton = {
                TextButton(onClick = {
