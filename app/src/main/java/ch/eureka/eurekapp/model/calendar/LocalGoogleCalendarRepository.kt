...
/**
 * Repository for interacting with the local Google Calendar on the device.
 * Provides functions to create, read events, and retrieve attendees and reminders.
 *
...
class LocalGoogleCalendarRepository(
    ...
    private object CompanionLocalGoogleCalendarRepository {
        ...
        val eventAttendeesProjection = arrayOf(
        ...
        val eventRemindersProjection = arrayOf(
    ...
    override suspend fun getCalendarEvent(contentResolver: ContentResolver, uniqueEventId: String):
        ...
        return Result.success(calendarEventData.copy(
            reminders = reminders,
            attendees = attendees
        ))
    }
     ...
     */
    private fun getEventFromGoogleCalendar(contentResolver: ContentResolver,
                                           uniqueEventId: String): Pair<Long, CalendarEventData> {
        ...
        val dateStart = eventCursor.getLong(
            CompanionLocalGoogleCalendarRepository.IDX_DTSTART)
        val dateEnd = eventCursor.getLong(
            CompanionLocalGoogleCalendarRepository.IDX_DTEND)
        val title = eventCursor.getString(
            CompanionLocalGoogleCalendarRepository.IDX_TITLE)
        val description = eventCursor.getString(
            CompanionLocalGoogleCalendarRepository.IDX_DESCRIPTION)
        val eventLocation = eventCursor.getString(
            CompanionLocalGoogleCalendarRepository.IDX_EVENT_LOCATION)
        val eventCalendarId = eventCursor.getLong(
            CompanionLocalGoogleCalendarRepository.IDX_EVENT_ID)
     ...
     */
    private fun getAttendeesFromEventId(contentResolver: ContentResolver, eventCalendarId: Long):
        ...
        while(attendeesCursor.moveToNext()){
            val attendeeName = attendeesCursor
                .getString(CompanionLocalGoogleCalendarRepository.IDX_ATTENDEE_NAME)
            val attendeeEmail = attendeesCursor
                .getString(CompanionLocalGoogleCalendarRepository.IDX_ATTENDEE_EMAIL)
            val attendeeRelationship = attendeesCursor.
            getInt(CompanionLocalGoogleCalendarRepository.IDX_ATTENDEE_RELATIONSHIP)
            val attendeeType = attendeesCursor
                .getInt(CompanionLocalGoogleCalendarRepository.IDX_ATTENDEE_TYPE)
            val attendeeStatus = attendeesCursor
                .getInt(CompanionLocalGoogleCalendarRepository.IDX_ATTENDEE_STATUS)
     ...
     */
    private fun getRemindersFromEventId(contentResolver: ContentResolver, eventCalendarId: Long):
        ...
        while(remindersCursor.moveToNext()){
            val minutesBefore = remindersCursor
                .getInt(CompanionLocalGoogleCalendarRepository.IDX_REMINDER_MINUTES)
            val method = remindersCursor
                .getInt(CompanionLocalGoogleCalendarRepository.IDX_REMINDER_METHOD)
    ...
    private suspend fun getGoogleCalendarID(contentResolver: ContentResolver): Int {
        ...
        check(user != null)
        val selectionQuery = "((${CalendarContract.Calendars.ACCOUNT_TYPE} = ?) AND (${
            CalendarContract.Calendars.OWNER_ACCOUNT} = ?))"
        val selectionArgs: Array<String> = arrayOf("com.google",user.email)

        val cur: Cursor? = contentResolver.query(CompanionLocalGoogleCalendarRepository.calendarUri,
            CompanionLocalGoogleCalendarRepository.CALENDAR_PROJECTION,
            selectionQuery, selectionArgs, null)
        check(cur != null)
        var googleCalendarID: Int?  = null
        cur.moveToFirst()
        val calID = cur.getInt(CompanionLocalGoogleCalendarRepository
            .PROJECTION_ID_INDEX)
     ...
     */
    private fun eventToContentValues(googleCalendarID: Int, eventData: CalendarEventData):
            ContentValues{
