...
object MeetingLinkValidator {
    ...
    private val WHITELISTED_DOMAINS = MeetingPlatform.values()
     ...
     *         - Valid non-whitelisted domains: (true, "Warning: This link is not from a trusted platform")
    ...
    fun detectPlatform(url: String?): MeetingPlatform {
        ...
        return MeetingPlatform.values()
            .find { platform ->
                platform.domains.any { platformDomain ->
                    domain.contains(platformDomain)
                }
    ...
    private fun isWhitelisted(domain: String): Boolean {
        return WHITELISTED_DOMAINS.any { whitelistedDomain ->
            domain.contains(whitelistedDomain)
        }
    }
}
