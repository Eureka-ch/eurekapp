...
class EurekaInfoCardCoverageTest {
  ...
  fun `EurekaInfoCard renders with minimal parameters`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = false) {
        EurekaInfoCard(title = "Minimal Title", primaryValue = "Minimal Value")
      }
    }
  ...
  fun `EurekaInfoCard renders with null secondaryValue`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = false) {
        EurekaInfoCard(title = "Title", primaryValue = "Primary", secondaryValue = null)
      }
    }
  ...
  fun `EurekaInfoCard renders in dark theme`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = true) {
        EurekaInfoCard(title = "Dark Title", primaryValue = "Dark Value")
      }
    }
  ...
  fun `EurekaInfoCard renders with empty strings`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = false) {
        EurekaInfoCard(title = "", primaryValue = "", secondaryValue = "")
      }
    }
  ...
  fun `EurekaInfoCard renders with long text`() {
    ...
    composeTestRule
        ...
        .assertIsDisplayed()
    composeTestRule
        .onNodeWithText("Very Long Primary Value That Should Still Render Correctly")
        .assertIsDisplayed()
    composeTestRule
        .onNodeWithText("Very Long Secondary Value That Should Still Render Correctly")
        .assertIsDisplayed()
  }
}
