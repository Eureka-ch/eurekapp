...
class EurekaTaskCardCoverageTest {
  ...
  fun `EurekaTaskCard renders with zero progress`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = false) {
        EurekaTaskCard(title = "Zero Progress Task", progress = 0.0f)
      }
    }
  ...
  fun `EurekaTaskCard renders with full progress`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = false) {
        EurekaTaskCard(title = "Full Progress Task", progress = 1.0f)
      }
    }
  ...
  fun `EurekaTaskCard renders in dark theme`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = true) { EurekaTaskCard(title = "Dark Theme Task") }
    }
  ...
  fun `EurekaTaskCard renders with empty strings`() {
    composeTestRule.setContent {
      EurekaTheme(darkTheme = false) {
        EurekaTaskCard(title = "", dueDate = "", assignee = "", priority = "", category = "")
      }
    }
  ...
  fun `EurekaTaskCard renders with long text`() {
    ...
    composeTestRule
        .onNodeWithText("Very Long Task Title That Should Still Render Correctly")
        .assertIsDisplayed()
  }
  ...
  fun `EurekaTaskCard renders with different progress values`() {
    ...
    progressValues.forEach { progress ->
      composeTestRule.setContent {
        EurekaTheme(darkTheme = false) {
          EurekaTaskCard(title = "Progress Task $progress", progress = progress)
        }
      }
