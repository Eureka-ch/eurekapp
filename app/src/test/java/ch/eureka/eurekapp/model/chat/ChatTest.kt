...
class ChatTest {
  ...
  fun chatChannel_defaultConstructor_createsEmptyChatChannel() {
    ...
    assertEquals("", channel.workspaceId)
    assertEquals("", channel.contextId)
    assertEquals(ChatContextType.WORKSPACE, channel.contextType)
  ...
  fun chatChannel_withParameters_setsCorrectValues() {
    val channel =
        ChatChannel(
            ...
            workspaceId = "ws123",
            contextId = "ctx123",
            contextType = ChatContextType.PROJECT,
            name = "General Discussion")

    assertEquals("ch123", channel.channelID)
    assertEquals("ws123", channel.workspaceId)
    assertEquals("ctx123", channel.contextId)
    assertEquals(ChatContextType.PROJECT, channel.contextType)
  ...
  fun chatChannel_copy_createsNewInstance() {
    ...
    assertEquals("ws123", copiedChannel.workspaceId)
  ...
  fun chatChannel_equals_comparesCorrectly() {
    ...
    val channel3 =
        ChatChannel(
            channelID = "ch456", workspaceId = "ws456", contextId = "ctx456", name = "Random")
  ...
  fun chatChannel_toString_containsAllFields() {
    ...
    assert(channelString.contains("ws123"))
    assert(channelString.contains("General Discussion"))
  }

  @Test
  fun chatContextType_hasAllValues() {
    val values = ChatContextType.values()

    assertEquals(3, values.size)
    assert(values.contains(ChatContextType.WORKSPACE))
    assert(values.contains(ChatContextType.GROUP))
    assert(values.contains(ChatContextType.PROJECT))
  }
