...
class DateTimeVoteTest {
  ...
  fun dateTimeVote_withParameters_setsCorrectValues() {
    ...
    val vote = DateTimeVote(dateTime = timestamp, voters = voters)

    assertEquals(timestamp, vote.dateTime)
    assertEquals(2, vote.voters.size)
  ...
  fun dateTimeVote_copy_createsNewInstance() {
    ...
    val vote1 = DateTimeVote(dateTime = timestamp, voters = listOf("user1"))
    val vote2 = vote1.copy(voters = listOf("user1", "user2"))

    assertNotSame(vote1, vote2)
    assertEquals(timestamp, vote2.dateTime)
    assertEquals(2, vote2.voters.size)
  ...
  fun dateTimeVote_equals_comparesCorrectly() {
    ...
    val vote1 = DateTimeVote(dateTime = timestamp, voters = listOf("user1"))
    val vote2 = DateTimeVote(dateTime = timestamp, voters = listOf("user1"))
    val vote3 = DateTimeVote(dateTime = timestamp, voters = listOf("user1", "user2"))
  ...
  fun dateTimeVote_toString_containsAllFields() {
    val timestamp = Timestamp(Date(0L))
    val vote = DateTimeVote(dateTime = timestamp, voters = listOf("u1", "u2", "u3", "u4", "u5"))
    val voteString = vote.toString()

    assertTrue(voteString.contains("dateTime=Timestamp(seconds=0, nanoseconds=0)"))
    assertTrue(voteString.contains("voters=[u1, u2, u3, u4, u5]"))
  }
  ...
  fun dateTimeVote_components_returnCorrectValues() {
    ...
    assertEquals(2, vote.component2().size)
    assertEquals(voters, vote.component2())
  }
}
